...
type Step = 'basic' | 'details' | 'customize';
...
const AddHabitModal: React.FC<AddHabitModalProps> = ({ onClose, onAddHabit }) => {
    ...
    const steps: { id: Step; label: string; icon: string }[] = [
        ...
        { id: 'details', label: 'Detalhes', icon: '⚙️' },
    ...
    const canProceedFromBasic = name.trim().length >= 3;
    const canProceedFromDetails = habitType === 'boolean' || (habitType === 'numeric' && targetValue && parseFloat(targetValue) > 0);
    ...
    const handleNext = () => {
        if (currentStep === 'basic') {
            ...
            if (!canProceedFromDetails) {
                setError('Por favor, defina uma meta válida para hábitos numéricos.');
                return;
            }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!canProceedFromBasic || !canProceedFromDetails) {
            ...
            >
                ...
                <div className="px-6 pt-6 pb-4 border-b border-slate-700/50">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            <h2 className="text-2xl font-bold text-white mb-1">Novo Hábito</h2>
                            <p className="text-sm text-slate-400">Crie seu hábito em passos simples</p>
                        </div>
                        <button 
                            onClick={onClose} 
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto custom-scrollbar px-6 py-6">
                    ...
                    {currentStep === 'basic' && (
                        <div className="space-y-6 animate-fade-in">
                            {/* Preview do hábito */}
                            <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-3xl p-8 border border-slate-700/50 text-center">
                                ...
                                <div className="text-sm text-slate-400">
                                    {habitType === 'boolean' ? 'Hábito Sim/Não' : `${targetValue || '0'} ${unit !== 'none' ? unit : ''} diários`}
                                </div>
                            </div>

                            <div>
                                <label htmlFor="habit-name" className="block text-sm font-semibold text-slate-300 mb-3 flex items-center gap-2">
                    ...
                    {currentStep === 'details' && (
                        <div className="space-y-6 animate-fade-in">
                            {habitType === 'numeric' && (
                                <>
                                    <div>
                                        <label htmlFor="habit-unit" className="block text-sm font-semibold text-slate-300 mb-3 flex items-center gap-2">
                                            <span className="w-6 h-6 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-400 text-xs font-bold">1</span>
                                            Unidade de Medida
                                        </label>
                                        <select
                                            id="habit-unit"
                                            ...
                                            className="w-full bg-slate-800/50 border-2 border-slate-700/50 rounded-2xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all duration-300 appearance-none cursor-pointer text-lg"
                                        >
                                            {HABIT_UNITS.map((u) => (
                                    ...
                                    <div>
                                        <label htmlFor="habit-target" className="block text-sm font-semibold text-slate-300 mb-3 flex items-center gap-2">
                                            <span className="w-6 h-6 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-400 text-xs font-bold">2</span>
                                            Meta Diária
                                        </label>
                                        <div className="relative">
                                            <input
                                                ...
                                                className="w-full bg-slate-800/50 border-2 border-slate-700/50 rounded-2xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all duration-300 text-lg pr-16"
                                                placeholder="0"
                                                required
                                            />
                                            {unit !== 'none' && (
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-slate-300 mb-3 flex items-center gap-2">
                                    <span className="w-6 h-6 rounded-lg bg-purple-500/10 flex items-center justify-center text-purple-400 text-xs font-bold">3</span>
                                    Período do Dia
                                </label>
                                <div className="grid grid-cols-2 gap-3">
                ...
                <div className="px-6 py-5 border-t border-slate-700/50 bg-slate-900/50 flex items-center justify-between gap-3">
                    {currentStepIndex > 0 ? (
                        <button
                            type="button"
                            onClick={handleBack}
                            className="px-6 py-3 bg-slate-800/50 hover:bg-slate-700/50 border-2 border-slate-700/50 hover:border-slate-600/50 text-white font-semibold rounded-2xl transition-all duration-300 active:scale-95 flex items-center gap-2"
                        >
                    ...
                    {currentStepIndex < steps.length - 1 ? (
                        <button
                            ...
                            className={`flex-1 px-6 py-3 font-bold rounded-2xl transition-all duration-300 active:scale-95 flex items-center justify-center gap-2 ${
                                (currentStep === 'basic' && canProceedFromBasic) || (currentStep === 'details' && canProceedFromDetails)
                                    ? 'bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white shadow-xl shadow-cyan-500/30 hover:shadow-cyan-500/50'
                                    : 'bg-slate-800/50 text-slate-500 cursor-not-allowed'
                            }`}
                            disabled={!((currentStep === 'basic' && canProceedFromBasic) || (currentStep === 'details' && canProceedFromDetails))}
                        >
                            Continuar
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                        ...
                        <button
                            ...
                            className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-bold py-3 px-6 rounded-2xl transition-all duration-300 shadow-xl shadow-emerald-500/30 hover:shadow-emerald-500/50 active:scale-95 flex items-center justify-center gap-2"
                        >
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
